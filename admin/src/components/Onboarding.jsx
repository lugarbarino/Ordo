...
import { PexelsPicker } from './ui/PexelsPicker'
    ...
    <div className="w-full max-w-md">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            <input type={showPass ? 'text' : 'password'} value={pass} onChange={e => setPass(e.target.value)}
              ...
              className="w-full px-4 py-3 pr-11 border border-[#e3e3e3] rounded-xl text-sm outline-none focus:border-[#295e4f] transition-colors bg-white" />
          ...
          <div className="text-sm text-red-500">
            ...
            {yaExiste && (
              <button onClick={onLogin}
                className="ml-2 underline font-semibold text-[#295e4f] bg-none border-none cursor-pointer">
                Iniciá sesión
              </button>
            )}
          </div>
        ...
        <p className="text-center text-xs text-[#aaa]">
          ¿Ya tenés cuenta?{' '}
          <button onClick={onLogin} className="text-[#295e4f] font-semibold underline bg-none border-none cursor-pointer">
            Iniciá sesión
          </button>
        </p>
      </div>
...
function Paso2({ onNext }) {
  ...
  const subirLogo = async (file) => {
    ...
    try {
      ...
      setLogoUrl(`${SUPABASE_URL}/storage/v1/object/public/logos/${path}`)
    } catch (e) {
    ...
    <div className="w-full max-w-md">
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          >
            {uploading ? (
              ...
              <div className="flex flex-col items-center gap-1.5 text-[#aaa]">
                <Upload size={20} />
                <span className="text-xs">PNG o SVG recomendado</span>
              </div>
            )}
          </div>
          ...
          {!logoUrl && !uploadError && <p className="text-xs text-[#bbb] mt-1">Si no subís logo, se mostrará el nombre de tu empresa.</p>}
        </div>
        ...
        <div>
          ...
          <div className="flex items-center gap-3">
            <div
              className="flex-1 h-11 rounded-xl cursor-pointer border border-[#e3e3e3]"
              style={{ background: color }}
              onClick={() => document.getElementById('ob-color').click()}
            />
            <input id="ob-color" type="color" value={color} onChange={e => setColor(e.target.value)}
...
function Paso3({ onFinish }) {
  ...
  const bannerIsVideo = isVideo(bannerUrl)
    ...
    <div className="w-full max-w-md">
      ...
      <div className="space-y-4">
        ...
        <div className="relative rounded-xl overflow-hidden bg-[#35425F]" style={{ height: 160 }}>
          {bannerIsVideo && bannerUrl ? (
            <video key={bannerUrl} autoPlay muted loop playsInline className="absolute inset-0 w-full h-full object-cover">
...
export function Onboarding({ initialStep = 1, onLogin }) {
  const { user, guardarEmpresa } = useAppStore()
  const [step, setStep] = useState(initialStep)
  const [empresaData, setEmpresaData] = useState({})

  const handleStep1 = ({ email }) => {
    setEmpresaData(d => ({ ...d, email }))
    // After signup, onAuthStateChange fires → App.jsx shows Onboarding at step 2
    // But if we're already here (Landing's Onboarding), we advance manually
    setStep(2)
  }
  ...
  const handleStep3 = async (bannerData) => {
    ...
    const campos = {
      ...empresaData,
      ...bannerData,
      email: empresaData.email || currentUser?.email,
    }
    await guardarEmpresa(campos)
    // empresa is now set in store → App.jsx renders Layout
  }
